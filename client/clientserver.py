...
def save_file(message, folderpath):
    ...
    with open(file_path, 'wb') as f:
        f.write(filedata)
    print(f'clientserver::handle_message: File {filename} saved successfully.')
...
def makeMessage(username, sender, receiver, commands, agentprompt, direction, content, client_id):
    ...
    print(f"clientserver::makeMessage: content = {content}.")
...
def handle_message(data):
    ...
    try:
        ...
        if data["type"] == "join":
            ...
            print(f'{username} is joining the room {room}.')

            join_room(room)
            if username not in userlist:
                userlist.append(username)

            print(f"Userlist is now {userlist}")
            ...
            for message_str in messages:
                ...
                if direction == "request":
                    emit('message', {'type': 'message', 'username': sender, 'message': message, 'timestamp': datetimeStr, "undo_hourglass": "true"}, broadcast=True, include_self=True)
            ...
            if len(messages) == 0:
                ...
                print(f"clientserver::handle_message adding special message: {message_str}")

            for message_str in messages:
                print(f"clientserver::handle_message putting message on input queue: {message_str}")
        ...
        def threadloop(stop_event, socketio):
            ...
            while not stop_event.is_set():
                    ...
                    print("clientserver::threadloop: at beginning")
                    ...
                    try:
                        ...
                        if input_queue.qsize() > 0:
                            print("clientserver::threadloop: input queue not empty")
                        ...
                        print(f"clientserver::threadloop: message_str = {message_str}")
                        ...
                        for response in responses:
                            ...
                            if (direction != "request" and direction != "request_noresponse") and len(content) > 0:
                                ...
                                print(f"chatserver::handle_server: roomtype is {roomtype} and content is {content}")

                                if roomtype == "chatroom" and "<img " in content:
                                    socketio.emit('image_message', {'type': 'image', 'username': username, 'message': message, 'timestamp': datetimeStr, "undo_hourglass": "true"})
                                else:
                                    emit('message', {'type': 'message', 'username': username, 'message': message, 'timestamp': datetimeStr, "undo_hourglass": "true"}, broadcast=True, include_self=True)
                            

                    except Exception as e:

                        print(f"clientserver::threadloop: Serious error: {str(e)}")
                        break
...
def inject():
    ...
    if data:
        ...
        socketio.emit('function_message', data)
        return '{"message": "Data received and emitted"}', 200
    else:
...
if __name__ == '__main__':
    ...
    print("Starting WSGI server with Eventlet on http://127.0.0.1:5000")
