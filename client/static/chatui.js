        ...
        function printMessage(username, message, timestamp) {

            console.log("printMessage: timestamp = "+ timestamp);

            var length = quill_chat.getLength();

            quill_chat.insertText(length - 1, username + ' [' + timestamp + '] ' + ': ', {bold:true});
        ...
        socket.on('message', function(data) {
            ...
            if (('type' in data) && (data.type == 'join')) {
                ...
                printMessage(data.username, content, data.timestamp);
            }
