...
from drivers.OpenAIDriver import COpenAIDriver
...
class CPacketManager:
	...
	def initialize(self):
		...
		loaded_modules = load_modules_from_directory("drivers")

		#driver_name = "OllamaDriver"

		driver_name = "OpenAIDriver"
		if driver_name is not None and driver_name in loaded_modules:
			driverfactory = loaded_modules[driver_name]
			if hasattr(driverfactory, "create_driver"):
				self.driver = driverfactory.create_driver(self.configurator)
		else:
			self.driver = COpenAIDriver(self.configurator)
		...
		if (len(self.packet_queue) >= self.MAX_QUEUE_SIZE):
			...
			if self.driver.isReady(): 
				...
				if self.isGeneralRequest(stream):
					...
					print()
					print(f"CPacketManager::processStream: the current before time is {beforeDatetime}")
					print()
					...
					print()
					print(f"CPacketManager::processStream: the current after time is {afterDatetime}")
					print(f"CPacketManager::processStream: the elapsed time in seconds is {elapsedTime}")
					print()
