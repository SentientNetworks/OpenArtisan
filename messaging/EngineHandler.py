...
class CEngineHandler:
    ...
    def api_process(cls, payload, server_url):
        ...
        try:
            ...
            _print(f"8. CEngineHandler::api_process: server_url is {server_url} and payload is {payload}")
            ...
            _print(f"9. CEngineHandler::api_process: the response is {response.json()}")
    ...
    def threadloop(cls, stop_event, out_handler, in_handler):
        ...
        while not stop_event.is_set():
                ...
                try:
                    ...
                    _print(f"CEngineHandler::handler: Serious error in handler loop: {str(e)}")
                    break
    ...
    def handle(cls, out_handler, in_handler, messages = None):
        ...
        _print(f"CEngineHandler::handle: start of handle loop before get")

        # Retrieve processed messages from the output queue
        message = out_handler.output_queue.get()
        messages.append(message)
            
        _print(f"CEngineHandler::handler: Processed message:{message}")    
        ...
        _print(f"7. CEngineHandler::handle: : after get before api_process: payload is {payload}")

        responses_str = cls.api_process(payload, ENGINE_SERVER_URL)
        _print(f"CEngineHandler::handle:  outin responses_str is {responses_str}")
        d = json.loads(responses_str)
        responses = d["processed_messages"]
        _print(f"10. CEngineHandler::handle: outin responses is {responses}")

        for response in responses:
            _print(f"CEngineHandler::handle: outin Processed response:{response}")
            in_handler.input_queue.put(response)

        _print(f"CEngineHandler::handle: outin")
