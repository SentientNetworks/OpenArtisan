...
class CFunctionHandler:
    ...
    def api_process(cls, payload, server_url):
        ...
        try:
            ...
            print(f"8. CFunctionHandler::api_process: server_url is {server_url} and payload is {payload}")
            ...
            print(f"9. CFunctionHandler::api_process: the response is {response.json()}")
    ...
    def threadloop(cls, stop_event, out_handler, in_handler):
        ...
        while not stop_event.is_set():
                ...
                try:
                    ...
                    print(f"CFunctionHandler::threadloop: Serious error in handler loop: {str(e)}")
                    break
    ...
    def handle(cls, out_handler, in_handler, messages = None):
        ...
        print(f"CFunctionHandler::handle: start of handle loop before get")

        # Retrieve processed messages from the output queue
        message = out_handler.output_queue.get()
        messages.append(message)
            
        print("CFunctionHandler::handler: Processed message:", message)    
        while not out_handler.output_queue.empty():
            message = out_handler.output_queue.get()
            messages.append(message)
                
            print("CFunctionHandler::handle: Processed message:", message)

        payload = {
                "messages": messages
        }    

        print(f"7. CFunctionHandler::handle: : after get before api_process: payload is {payload}")

        responses_str = cls.api_process(payload, FUNCTION_SERVER_URL)
        print(f"CFunctionHandler::handle:  outin responses_str is {responses_str}")
        d = json.loads(responses_str)
        responses = d["processed_messages"]
        print(f"10. CFunctionHandler::handle: outin responses is {responses}")

        for response in responses:
            print("CFunctionHandler::handle: outin Processed response:", response)
            in_handler.input_queue.put(response)

        print(f"CFunctionHandler::handle: outin")
