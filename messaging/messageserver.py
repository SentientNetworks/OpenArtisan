...
from MessageRouter import router
import Handlers
from Handlers import queue_handlers, ENGINE_HANDLER, CLIENT_HANDLER, MEMORY_HANDLER, FUNCTION_HANDLER, TIMER_HANDLER, QUEUE_HANDLER_NAMES
...
def process_messages():
    ...
    print(f"2. messageserver::process_messages: data is {data}")

    messages = data["messages"]

    print(f"messageserver::process_messages: messages are {messages}")
    ...
    print(f"3. messageserver::process_messages: before placing on in queue for handler name {handler_name} and handler_type {handler_type}")

    responses = handler.handle(queue_handlers[CLIENT_HANDLER][0], queue_handlers[CLIENT_HANDLER][0], messages = messages)

    print(f"12. messageserver::process_messages: the responses are {responses}")
...
if __name__ == '__main__':
    ...
    routerthread = threading.Thread(target=router, args=(stop_event,queue_handlers,QUEUE_HANDLER_NAMES,))
    routerthread.daemon = True  # Daemonize thread to ensure it exits when main program exits
    routerthread.start()
