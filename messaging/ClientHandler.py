...
class CClientHandler:
    ...
    def handle(cls, out_handler, in_handler, messages = None):
            ...
            projector_tag, messages = businesslogic.check_for_projector_tag(messages)
 
            _print(f"CClientHandler::handle: client_tag is {client_tag} and projector_tag is {projector_tag}")

            _print(f"CClientHandler::handle: type is inout and messages is {messages}")            
            ...
            for message_str in messages:            
                ...
                _print("")
                _print(f"messaging::CClientHandler::handle: current time [timing] after inserting messages on input queue is {currentTime}")
                _print("")
            ...
            for message_str in messages:
                ...
                _print(f"messaging::CClientHandler::handle: redirecting message {redirected_message_str}")
                in_handler.input_queue.put(redirected_message_str)
            
            responses = []

            _print("CClientHandler::handle: ")   
            ...
            # We only need to grab one response message at a time
            # since there is a larger loop which will call this function
            # so all the response strings should be captured eventually
            response_str = out_handler.output_queue.get_nowait()

            #response_str = businesslogic.redirect_to_client(response_str)

            ## TODO add the projector_tag to the method
            keep_response, response_str = businesslogic.belongs_to_client(client_tag, projector_tag, response_str)

            _print(f"CClientHandler::handle: keep_response = {keep_response}, client tag = {client_tag}, response_str = {response_str}")
            ...
            _print(f"11. CClientHandler::handle: Processed response:{response_str}")    
